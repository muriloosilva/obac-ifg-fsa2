...
public class ControleMolaMouse {
	...
	private final MouseListener mListener =new MouseAdapter() {
		...
		public void mouseReleased(MouseEvent arg0) {
			...
			if(areaDaMola && ma.getmP().getTamanhoMolaAtualPix()<ma.getmP().getTamanhoMolaTotalPix()){
	...
	private void reposicionaObjeto(int x, int y){
		...
		areaDaMola =(
			...
			(x>=(vp.getPosIniXProp()+ma.getmP().getTamanhoMolaMinimoPix())&& x<=(vp.getPosIniXProp()+ma.getmP().getTamanhoMolaTotalPix()))
			 //Confirma se a posição do mouse no eixo Y está correta
			&&(y>=vp.getPosIniYProp() && y<=(vp.getPosIniYProp()+ma.getmO().altura))
		);
		...
		if(areaDaMola){
			...
			ma.getmP().setTamanhoMolaAtualPix(x-vp.getPosIniXProp());
			ma.getmP().setTamanhoMolaTotalM((UtilidadeConvercoesEscala.convertePixelMetro(ma.getmP().getTamanhoMolaTotalM(), ma.getmP().getTamanhoMolaAtualPix(), ma.getmP().getTamanhoMolaTotalPix())));
			//Altera o tamanho da imagem para ajustála a compressão
			vp.setImagemPropulsao(new ImageIcon(ma.getmP().getImagemMola().getImage().getScaledInstance(ma.getmP().getTamanhoMolaAtualPix(), 30, Image.SCALE_DEFAULT)));
